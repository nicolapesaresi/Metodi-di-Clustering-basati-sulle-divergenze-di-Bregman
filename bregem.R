bregem<-function(dati, mu=NULL, mixprobs=NULL, K=2, hard=T, ciclimax=1000, sogliaconvergenza=0.00001)
{
  #inizializzazione
  stime<-list()
  ...
  #nel caso normale e' 1/2 ||x-mu||^2
  bregdiv<-function(x, media){
    #as.double((as.matrix(xx-mux))%*%t(as.matrix(xx-mux)))
    return(0.5*dist(rbind(x,media))^2)
  }
  ...
  {stime[[1]]$mixprobs[i]*(1/sqrt(2*pi))*exp(-bregdiv(rigaj,stime[[1]]$mu[[i]]))})})))))
  ...
  while(convergenza==F & c<=ciclimax) {
    ...
    {apply(matrix(1:K,ncol=1),1,function(i) {stime.old$mixprobs[i]*exp(-bregdiv(rigaj,stime.old$mu[[i]]))})}))
    ...
    {apply(matrix(1:K,ncol=1),1,function(i) 
    {stime[[c]]$mixprobs[i]*(1/sqrt(2*pi))*exp(-bregdiv(rigaj,stime.old$mu[[i]]))})})))))
  ...
  {apply(matrix(1:K,ncol=1),1,function(i) 
  {stime[[c-1]]$mixprobs[i]*dens(rigaj,stime[[c-1]]$mu[[i]])})})))))
