...
library(mclust)
tab3<-tab
tab3[,3]<-Mclust(tab[,-3],G=3)$classification

#cambio numerazione per avere stessi colori
temp<-tab3[,3]
temp[which(tab3[,3]==1)]<-2
temp[which(tab3[,3]==2)]<-1
temp[which(tab3[,3]==3)]<-3
tab3[,3]<-temp

gr3<-ggplot(tab3, aes(x = tab3[,1], y = tab3[,2])) +
  theme_light() +
  xlab("x") + ylab("y") +
  theme(axis.text.y=element_blank(),axis.text.x=element_blank(),panel.grid.major.y = element_blank() ) +
  geom_point(color=tab3[,3]+1) +
  ggtitle("Cluster GMM")

library(cowplot)
plot_grid(gr1,gr2,gr3,nrow=1)